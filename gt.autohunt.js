...
// @author         MuzTank & GTDevsSuck
// @namespace      Original versions by Jaryl & iispyderii
// @version        4.07
// @updateURL      https://www.yohanesedwin.com/script/gt.autohunt.js
...
// @history        4.07 ::: Added ability to choose Gamble Ghost's cup
// @history        4.06 ::: Testing version update
// @history        4.05 ::: Added monster checking on ALL visible log.
// @history        4.04 ::: Added Auto Video Watcher
// @history        4.03 ::: Added OPTIONS which you can edit by buttons placed at the bottom of the page.
// @history        4.02 ::: Monster auto-place in live feed option.
// @history        4.01 ::: Captcha auto-submit attempt option.
...
{
  if(document.getElementById("croupierButton1") != null) {
    var randInt = Math.floor(Math.random() * (3 - 1 + 1)) + 1;
    // skipGamble();
    croupierGamble(randInt);
    titlePlaceHolder = "You got Gamble ghost!";
    console.log("You got Gamble Ghost and choose cup number #"+randInt);
  }
  ...
  if (drinkCount <= 6) {
    ...
    var minutesid, secondsid;
      ...
      for(var i=0; i<$(".logText").length; i++) {
          var checkMonster = $($($('.logText'))[i]).html();
          if (checkMonster.match(/is too strong for you alone/i))
          {
              if (localStorage.monsterAlert === "true" && localStorage.lastMonsterLog !== document.getElementsByClassName('logText')[i].innerHTML) {
                  setTimeout(function(){	localStorage.lastMonsterLog = document.getElementsByClassName('logText')[i].innerHTML;
                                        alert("You got a monster, you might want to send it to live feed (or remove it "
                                              + "with irrelevance).\n\nGood luck!");}, 500);
              }
              if (localStorage.monsterHunt === "true" && localStorage.lastMonsterLog !== document.getElementsByClassName('logText')[i].innerHTML) {
                  setTimeout(function(){
                      localStorage.lastMonsterLog = document.getElementsByClassName('logText')[i].innerHTML;
                      $($(".logPost")[i]).find("a").click();
                      console.log("Monster found @"+i);
                  }, 750);
              }
              break;
          }
...
if(typeof(Storage) !== "undefined") {
  ...
  function titleButton03() {
    ...
    if (localStorage.monsterHunt == "true") {
      ...
      t=document.createTextNode("Monster Alert/Auto-Hunt are Disabled");
    }
    button03.innerHTML = "";
      ...
      $.ajax({
        ...
        success: function(data) {
          if(data.remainingVideos == 0) {
            clearInterval(xxx);
            console.log("Auto Video Chrono have been used today. Please try again tomorrow.");
            $('button:eq(6)').html('Auto Chrono Done');
          }
    else {
      $('button:eq(6)').html('Remaining Videos: '+data.remainingVideos);
    }
        }
      });
      i++;
